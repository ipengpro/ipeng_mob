...
erpnext.EmployeeSelector = Class.extend({
	...
	make: function(frm, employee) {
		...
		var mark_employee_toolbar = $('<div class="col-sm-12 bottom-toolbar">\
			...
			<button class="btn btn-warning btn-mark-unpaid btn-xs" hidden></button>\
			<button class="btn btn-warning btn-mark-sick btn-xs" hidden></button>\
			<button class="btn btn-warning btn-mark-privilege btn-xs" hidden></button>\
			</div>');
		...
		mark_employee_toolbar.find(".btn-mark-absent")
           ...
           if (frappe.user.has_role('Security Guard Manager')) {
				
				mark_employee_toolbar.find(".btn-mark-unpaid")
				.html(__('Mark Unpaid Leave'))
				.removeAttr('hidden')
				.on("click", function() {
					var employee_absent = [];
					$(me.wrapper).find('input[type="checkbox"]').each(function(i, check) {
				...
				mark_employee_toolbar.find(".btn-mark-sick")
				.html(__('Mark Sick Leave'))
				.removeAttr('hidden')
				.on("click", function() {
					var employee_absent = [];
					$(me.wrapper).find('input[type="checkbox"]').each(function(i, check) {
				...
				mark_employee_toolbar.find(".btn-mark-privilege")
				.html(__('Mark Privilege Leave'))
				.removeAttr('hidden')
				.on("click", function() {
					var employee_absent = [];
					$(me.wrapper).find('input[type="checkbox"]').each(function(i, check) {
